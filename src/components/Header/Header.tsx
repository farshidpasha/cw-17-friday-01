...
export default function Header() {
  const [isModifyModalOpen, setIsModifyModalOpen] = useState({ isOpen: false, id: null });
  const openModal = () => {};
  ...
  return (
    <>
      {isModifyModalOpen.isOpen && (
        <Modal title="Modal" content={<ModifyModalContent />} closeHandle={closeModifyModalHandler} />
      )}
      <header className="p-4 flex justify-between items-center bg-[#6200ea]">
        ...
        <div className="flex gap-4">
          <Button className="w-9" icon={<FaPlus color="#6200ea" />} onClick={addTaskHandler} />
